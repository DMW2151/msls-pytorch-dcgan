...
class BigDiscriminator(nn.Module):
    def __init__(self, cfg):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(nc, cfg.ndf, 4, 2, 1, bias=False),
            nn.LeakyReLU(0.2, True),
            # state size. (ndf) x 64 x 64
            nn.Conv2d(ndf, cfg.ndf * 2, 4, 2, 1, bias=False),
            nn.BatchNorm2d(ndf * 2),
            nn.LeakyReLU(0.2, True),
            # state size. (ndf*2) x 32 x 32
            nn.Conv2d(ndf * 2, cfg.ndf * 4, 4, 2, 1, bias=False),
            nn.BatchNorm2d(ndf * 4),
            nn.LeakyReLU(0.2, True),
            # state size. (ndf*4) x 16 x 16
            nn.Conv2d(ndf * 4, cfg.ndf * 8, 4, 2, 1, bias=False),
            nn.BatchNorm2d(ndf * 8),
            nn.LeakyReLU(0.2, True),
            # state size. (ndf*8) x 8 x 8
            nn.Conv2d(ndf * 8, cfg.ndf * 16, 4, 2, 1, bias=False),
...
class BigGenerator(nn.Module):
    def __init__(self, cfg):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(ngf * 16, cfg.ngf * 8, 4, 2, 1, bias=False),
            nn.BatchNorm2d(ngf * 8),
            nn.ReLU(True),
            # state size. (ngf*8) x 8 x 8
            nn.ConvTranspose2d(ngf * 8, cfg.ngf * 4, 4, 2, 1, bias=False),
            nn.BatchNorm2d(ngf * 4),
            nn.ReLU(True),
            # state size. (ngf*4) x 16 x 16
            nn.ConvTranspose2d(ngf * 4, cfg.ngf * 2, 4, 2, 1, bias=False),
            nn.BatchNorm2d(ngf * 2),
            nn.ReLU(True),
            # state size. (ngf*2) x 32 x 32
            nn.ConvTranspose2d(ngf * 2, cfg.ngf, 4, 2, 1, bias=False),
            nn.BatchNorm2d(ngf),
            nn.ReLU(True),
