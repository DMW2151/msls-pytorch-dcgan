...
class Generator128(nn.Module):
    def __init__(self, cfg):
        ...
        self.main = nn.Sequential(
            ...
            nn.ReLU(True),
            # state size. (cfg.ngf*16) x 4 x 4
            nn.ConvTranspose2d(cfg.ngf * 16, cfg.ngf * 8, 4, 2, 1, bias=False),
            nn.BatchNorm2d(cfg.ngf * 8),
            nn.ReLU(True),
            # state size. (cfg.ngf*8) x 8 x 8
            nn.ConvTranspose2d(cfg.ngf * 8, cfg.ngf * 4, 4, 2, 1, bias=False),
            nn.BatchNorm2d(cfg.ngf * 4),
            nn.ReLU(True),
            # state size. (cfg.ngf*4) x 16 x 16
            nn.ConvTranspose2d(cfg.ngf * 4, cfg.ngf * 2, 4, 2, 1, bias=False),
            nn.BatchNorm2d(cfg.ngf * 2),
            nn.ReLU(True),
            # state size. (cfg.ngf*2) x 32 x 32
            nn.ConvTranspose2d(cfg.ngf * 2, cfg.ngf, 4, 2, 1, bias=False),
            nn.BatchNorm2d(cfg.ngf),
            nn.ReLU(True),
            # state size. (cfg.ngf) x 64 x 64
            nn.ConvTranspose2d(cfg.ngf, cfg.nc, 4, 2, 1, bias=False),
            nn.Tanh()
...
class Discriminator128(nn.Module):
    def __init__(self, cfg):
        ...
        self.main = nn.Sequential(
            ...
            nn.BatchNorm2d(cfg.ndf * 2),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (cfg.ndf*2) x 32 x 32
            nn.Conv2d(
                cfg.ndf * 2, cfg.ndf * 4, 4, stride=2, padding=1, bias=False
            ),
            nn.BatchNorm2d(cfg.ndf * 4),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (cfg.ndf*4) x 16 x 16
            nn.Conv2d(
                cfg.ndf * 4, cfg.ndf * 8, 4, stride=2, padding=1, bias=False
            ),
            nn.BatchNorm2d(cfg.ndf * 8),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (cfg.ndf*8) x 8 x 8
            nn.Conv2d(
                cfg.ndf * 8, cfg.ndf * 16, 4, stride=2, padding=1, bias=False
            ),
            nn.BatchNorm2d(cfg.ndf * 16),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (cfg.ndf*16) x 4 x 4
